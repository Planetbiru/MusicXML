...
use MusicXML\Model\Dot;
use MusicXML\Model\Duration;
use MusicXML\Model\Instrument;
...
class MusicXMLFromMidi extends MusicXMLBase
{
     ...
     *
     * @deprecated 1.0
     * @param int $ch The MIDI channel number.
     * @param int $indexOn The index of the measure.
     ...
     * @param int $programId The MIDI program ID (1-128).
     ...
     * @param int $port The MIDI port number.
     * @return void
     */
     ...
     * @param int $duration The calculated duration of the note in ticks.
     * @return boolean
     */
    ...
    public function midiToScorePartwiseObject($midi, $title, $version = "4.0")
    {
        ...
        foreach ($this->partList as $part) {
            ...
            if ($channelId == 10) {
                ...
                $instrumentList = MusicXMLInstrument::INSTRUMENT_LIST;
                $index = $programId - 1;

                $instrumentSound = null;
                if (isset($instrumentList[$index]) && isset($instrumentList[$index][2])) {
                    $instrumentSound = $instrumentList[$index][2];
                } else {
     ...
     * @param int $channelId The MIDI channel ID (should be 10).
     * @param int $programId The MIDI program ID for the drum kit.
     ...
     * @param int $measureIndex The index of the measure to check.
     * @return boolean
     */
     ...
     *
     * @param int $measureIndex The index of the measure being built.
     ...
     *
     * This method first identifies all lyric positions within the specified time gap. It then
     * treats these positions as boundaries, creating separate rest notes for each segment
     * between the boundaries. This ensures that lyrics are correctly attached to the
     * beginning of a rest, even if it's in the middle of a larger period of silence.
     *
    ...
    private function fillGapWithRestsV1($measure, $measureIndex, $divisions, $timebase, $xmlStart, $xmlEnd, &$lyricDivisions)
    {
        ...
        // Define standard rest durations in divisions
        $restValues = array(
            'whole' => $divisions * 4,
            'half' => $divisions * 2,
            'quarter' => $divisions,
        ...
        while ($remainingDuration > 0) {
            ...
            // Find the largest standard rest that fits
            foreach ($restValues as $type => $value) {
                if ($value <= $remainingDuration) {
                    $bestRestDuration = $value;
                    $bestRestType = $type;
                    break;
                }
            }

            if ($bestRestDuration == 0) {
                // If no standard rest fits (e.g., duration is too small), use the remaining duration
                $bestRestDuration = $remainingDuration;
            ...
            // Attach lyrics if they fall at the beginning of this rest
            foreach ($lyricDivisions as $lyricPos => $lyricText) {
                if ($lyricPos >= $currentXml && $lyricPos < ($currentXml + $bestRestDuration)) {
                    $rest->lyric = array($this->createLyricModel($lyricText));
                    unset($lyricDivisions[$lyricPos]);
                    break; // Attach only one lyric per rest note
                }
            }

            $measure->elements[] = $rest;
            $currentXml += $bestRestDuration;
            $remainingDuration -= $bestRestDuration;
        }
    }
        ...
        foreach ($boundaries as $nextXml) {
            ...
            if ($dur > 0) { // Only add rest if there is a duration
                ...
                if (isset($this->measureOnsets[$measureIndex][$absTime])) {
                    ...
                    $measW = isset($this->measureWidth[$measureIndex]) ? $this->measureWidth[$measureIndex] : 3000;
                    $ratio = ($measureLength > 0) ? (($absTime - $absMeasureStart) / $measureLength) : 0;
                    if ($ratio < 0) $ratio = 0;
                    if ($ratio > 1) $ratio = 1;
                    $rest->defaultX = 60.0 + $ratio * ($measW - 120.0);
                }
    ...
    private function trimNoteDuration($note, $newRawDuration, $divisions, $timebase)
    {
        if ($newRawDuration > 0) {
            ...
            unset($note->tie); // Pastikan tidak ada tie jika tidak meluap
        }
        return $note;
    }
    ...
    /**
     * Get the value of useRestFilling
     */
    public function getUseRestFilling()
    {
        return $this->useRestFilling;
    }

    /**
     * Set the value of useRestFilling
     * 
     * @param bool $useRestFilling 
     */
    public function setUseRestFilling($useRestFilling)
    {
